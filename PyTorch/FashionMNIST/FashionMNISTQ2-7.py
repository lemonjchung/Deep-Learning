...
# The highest 10 std of:  [0.00098287, 0.00093526, 0.00088557, 0.00086568, 0.00083635, 0.0008207, 0.00081859, 0.0008138, 0.00081226, 0.00081145]

# #  Code
# # --- Q2-7 Save Gradient to CSV file
# writer.writerow({'Gradients': images.grad.cpu().numpy()})
# writer.writerow({'STD': round(float(torch.std(images.grad)), 8)})
# grad_stdlist.append(round(float(torch.std(images.grad).cpu().numpy()), 8))
# # --- Q2-7 Save Gradient to CSV file
# # --- Q2-7 Save the highest STD Gradients to CSV file
# writer.writerow({'The highest 10 std': sorted(grad_stdlist, reverse=True)[:10]})
# # --- Q2-7 Save the highest STD Gradient to CSV file
...
# --------------------------------------------------------------------------------------------
start = time.time()
with open('Q2-7Gradients.csv', 'w') as csvfile:
    fieldnames = ['Gradients', "STD", 'The highest 10 std']
...
print("The highest 10 std of: ", sorted(grad_stdlist, reverse=True)[:10])
